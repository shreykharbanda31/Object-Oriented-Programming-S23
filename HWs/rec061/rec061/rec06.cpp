...
class Directory {
    ...
    Entry(const Entry& entr) : name(entr.name), room(entr.room), phone(entr.phone){
            pos = new Position(entr.pos->getTitle(), entr.pos->getSalary());
        }
...
public:
    Directory(const string& cname) : size(0), capacity(10), company(cname) {
        entries = new Entry*[capacity];
    };
    
    Directory(const Directory& dir) : size(dir.size), capacity(dir.capacity), company(dir.company){
        entries = new Entry*[capacity];
    ...
    ~Directory(){
        cout << "Deleting all entries from the directory" << endl;
        for (size_t index = 0; index < size; ++index){
            delete entries[index];
            entries[index] = nullptr;
        }
    ...
    void add(const string& name, unsigned room, unsigned ph, Position& pos) {
        ...
        if (size == capacity)    {
            ...
            for (size_t index = 0; index < size; ++index){
                new_entries[index] = new Entry(*entries[index]);
            }
            delete [] entries;
            entries = new_entries;
            capacity = capacity * 2;
