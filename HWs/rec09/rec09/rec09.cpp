...
void displayNumPages(const PrintedMaterial& rhs){
    rhs.displayNumPages();
};
...
//    PrintedMaterial pm = text; // like putting a Textbook into a PrintedMaterial
//    pm.displayNumPages();
    ...
    vector<PrintedMaterial*> vecP;
    vecP.push_back(&text);
    vecP.push_back(&novel);
    vecP.push_back(&mag);
    
    cout << "VECTOR" << endl;
    for(size_t index = 0; index < vecP.size(); ++index){
        vecP[index] -> displayNumPages();
    }
    cout << "===========\n";
    for (const PrintedMaterial* pmPtr : vecP) {
    pmPtr->displayNumPages();
    cout << endl;
    }
}
